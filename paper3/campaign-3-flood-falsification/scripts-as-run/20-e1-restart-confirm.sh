...
case "${ARM}" in
  ...
  prefix) BINSRC=/data/stargz-bin-prefix LABEL=prefix FMM="0.0.0.0:9111" ;;
  *) die "ARM must be sut or prefix" ;;
esac
...
observe() { # observe <tag>
  ...
  log "  [obs ${tag}] $(grep -E '^(config_policy|fm_pid|n9110_cache_series|policy_labels_9110|lock_lost_9110|db_holders)=' "${d}/observation.txt" | paste -sd' ' -)"
}
...
log "  --- restart 1: policy lru -> 2q ---"
set_policy_in_config 2q > "${OUT}/config-change-1.txt" 2>&1
plain_restart
observe 2-after-restart-to-2q

# a cycle has to actually run for a counter to carry the new label; give it one
log "  re-reading to drive an eviction cycle under the new policy"
full_read e1-warm "${WARM_FILES}" > "${OUT}/reread-2q.txt" 2>&1 || true
observe 3-after-read-under-2q

# 4. and back, so the label is shown to TRACK the config rather than to have moved once
log "  --- restart 2: policy 2q -> lru ---"
set_policy_in_config lru > "${OUT}/config-change-2.txt" 2>&1
plain_restart
observe 4-after-restart-to-lru
full_read e1-warm "${WARM_FILES}" > "${OUT}/reread-lru.txt" 2>&1 || true
observe 5-after-read-under-lru
