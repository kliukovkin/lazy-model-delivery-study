#!/usr/bin/env bash
# E3 [P1] -- the pressure matrix at 90%, N=2, both arms.
#
...
# a pass. See PRE-REGISTRATION-v6.md s3.
set -euo pipefail
. "$(dirname "$0")/env.sh"
. "$(dirname "$0")/lib.sh"
...
trial() { # trial <arm> <rep>
  local arm="$1" rep="$2"
  local out="${E3}/${arm}-${LEVEL}pct-rep${rep}"; mkdir -p "${out}"
  local pod="e3-${arm}-${rep}"
  log "=== E3 arm=${arm} prefill=${LEVEL}% rep=${rep} ==="
  ...
  { echo "experiment=E3"; echo "arm=${arm}"; echo "prefill_pct=${LEVEL}"; echo "rep=${rep}"
...
mkdir -p "${E3}"
log "E3: our build, ${REPS} reps at ${LEVEL}%"
switch_arm ours
for r in $(seq 1 "${REPS}"); do trial ours "${r}"; done

log "E3: vanilla ${STARGZ_VER} control, ${REPS} reps at ${LEVEL}% -- the load-bearing negative control"
switch_arm vanilla
for r in $(seq 1 "${REPS}"); do trial vanilla "${r}"; done

log "E3: restoring our arm"
switch_arm ours
...
column -s, -t < "${E3}/e3-summary.csv" | tee "${E3}/E3-RESULT.txt"
log "E3 done -> ${E3}"
