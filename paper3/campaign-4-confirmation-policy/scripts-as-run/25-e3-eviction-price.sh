#!/usr/bin/env bash
# E3 [P0] -- the price of eviction, on fixed code, with enough N to conclude.
#
...
ARM_SEQUENCE="${ARM_SEQUENCE:-lru 2q 2q lru lru 2q 2q lru}"
E3="${RESULTS_DIR}/e3"
NODE="$(NODE_CTR)"
...
armrun() { # armrun <seq-index> <policy>
  local idx="$1" pol="$2"
  local out="${E3}/run${idx}-${pol}"; mkdir -p "${out}"
  log "=== E3 run ${idx}: policy=${pol} ==="
  ...
  { echo "experiment=E3"; echo "run_index=${idx}"; echo "policy=${pol}"
  ...
  teardown_pod e3-resident; teardown_pod e3-sweep
  deploy_pod e3-resident "${RESIDENT_IMG}" normal 3
  wait_for 300 "resident ready" pod_ready e3-resident || die "resident never Ready"
  ...
  lat_probe e3-resident 0 "${HOT_FILES}" 1 > "${out}/warm-1.txt" 2>&1 || true
  ...
  for k in 2 3 4 5 6 7; do
    ...
    lat_probe e3-resident 0 "${HOT_FILES}" 1 > "${out}/warm-${k}.txt" 2>&1 || true
    if [ "$(grep -c 'errno=' "${out}/warm-${k}.txt" || true)" -gt 0 ]; then
      ...
      teardown_pod e3-resident; mv "${out}" "${out}-INVALID"; return 0
    fi
  ...
  deploy_pod e3-sweep "${SWEEP_IMG}" normal 3
  wait_for 300 "sweep ready" pod_ready e3-sweep || log "  WARN: sweep pod not Ready; continuing"
  ( full_read e3-sweep 280 > "${out}/sweep-read.txt" 2>&1 ) </dev/null >/dev/null 2>&1 &
  local SWEEPPID=$!

  log "  measuring resident latency, ${ROUNDS} rounds, under sweep pressure"
  lat_probe e3-resident 0 "${HOT_FILES}" "${ROUNDS}" > "${out}/resident-latency.txt" 2>&1 || true
  ...
  teardown_pod e3-sweep; teardown_pod e3-resident
  ...
  bash "$(dirname "$0")/91-collect-one.sh" "e3/run${idx}-${pol}" || log "  WARN: collection failed for run ${idx}"
}
...
log "=== E3 gate + analysis ==="
python3 "$(dirname "$0")/25-e3-analyse.py" "${E3}" | tee "${E3}/E3-RESULT.txt"
log "E3 done -> ${E3}"
