#!/usr/bin/env python3
"""E3 gate and analysis. PRE-REGISTRATION-v7.md s3.3 / s3.4.
...
for d in sorted(glob.glob(os.path.join(E3, "run*-*"))):
...
if len(runs) < 6:
    print("G0 FAIL  only %d valid arm-runs of 8; more than two were excluded" % len(runs))
    gate_ok = False
else:
    print("G0 PASS  %d of 8 arm-runs valid" % len(runs))
...
if not gate_ok:
    ...
    with open(os.path.join(E3, "GATE-FAILURE.md"), "w") as f:
    ...
    print("wrote %s/GATE-FAILURE.md" % E3)
    sys.exit(0)
...
print()
print("=== EFFECT SIZE (N=4 per policy; PRE-REGISTRATION-v7 s3.4) ===")
...
chk("E3.1", gate_ok, "G1 and G2 both passed")
chk("E3.4", all(r["errs"] == 0 for r in runs),
    "resident read errors per run = %s" % [r["errs"] for r in runs])
chk("E3.x", all(r["evictions"] > 0 for r in runs),
    "evictions per run = %s" % [int(r["evictions"]) for r in runs])
chk("E3.5", all(r["skipped"] == 0 for r in runs),
...
lab_ok = all(("lru" in r["labels_end"].split(",")) if r["policy"] == "lru"
             ...
             for r in runs)
chk("E3.6", lab_ok, "end-of-run policy labels = %s" % [(r["idx"], r["policy"], r["labels_end"]) for r in runs])
promoted = []
for r in runs:
    if r["policy"] != "2q":
        continue
...
chk("E3.6b", all(p[1] and p[2] > 0 for p in promoted) if promoted else False,
