#!/usr/bin/env bash
# spike-v5 node-host: install stargz-snapshotter INSIDE the kind node, under
...
NODE="$(NODE_CTR)"
# v5: BINSRC selects the arm.
#   /data/stargz-bin-ours -> our fork @ OUR_SHA   (system under test; default)
#   /data/stargz-bin      -> upstream v0.18.2     (vanilla control for V2/V4)
BINSRC="${BINSRC:-/data/stargz-bin-ours}"
ARM_LABEL="${ARM_LABEL:-ours}"
# v6: the manager's own metrics endpoint is now OPTIONAL and OFF by default.
# That is the whole of E1.1: the F2 fix claims the snapshotter's documented
# metrics_address federates the manager's series, so a rig that still sets this
# key cannot tell whether federation works or whether it is reading the manager
# directly. The negative-control arm sets it explicitly, because the pre-fix
# build has no reachable metrics at all without it.
FM_METRICS_ADDRESS="${FM_METRICS_ADDRESS:-}"
# v7 E2 drives this down to force the accounting queue to overflow: the C4
# defect needs a flooded queue, and a test that merely hopes for one is not a
# test. The shipped default is 8192.
CACHE_QUEUE_SIZE="${CACHE_QUEUE_SIZE:-8192}"
...
# The accounting/eviction stanza only exists on our build; v0.18.2 would reject
# nothing (unknown TOML keys are ignored) but writing it there would misrepresent
# what the control arm was running, so the control gets a config without it.
ACCOUNTING_STANZA=""
if [ "${ARM_LABEL}" != "vanilla" ] && [ "${CACHE_ACCOUNTING}" = "true" ]; then
  ACCOUNTING_STANZA=$(cat <<STANZA

[cache_accounting]
  enable = true
  queue_size = ${CACHE_QUEUE_SIZE}
[cache_accounting.budget]
  bytes = ${CACHE_BUDGET_BYTES}
  high_watermark = ${CACHE_HIGH_WM}
  low_watermark = ${CACHE_LOW_WM}
  policy = "${CACHE_POLICY}"
STANZA
)
fi
export ACCOUNTING_STANZA

# Written only when asked for. See the FM_METRICS_ADDRESS comment above.
FM_METRICS_LINE=""
if [ -n "${FM_METRICS_ADDRESS}" ]; then
  FM_METRICS_LINE="  metrics_address = \"${FM_METRICS_ADDRESS}\""
fi
export FM_METRICS_LINE

log "writing /etc/containerd-stargz-grpc/config.toml (arm=${ARM_LABEL} fuse_manager=${FUSE_MANAGER} fm_metrics=${FM_METRICS_ADDRESS:-<unset>})"
...
# v5 difference from v4: the KillMode=process drop-in is INSTALLED, not measured.
# v4 S1 established that the shipped unit (KillMode=control-group) SIGTERMs the
# detached FUSE manager and so silently defeats [fuse_manager], upstream issue
# #2387. Every v5 experiment assumes a snapshotter that survives its own restart,
# so the fix is part of the baseline configuration here and is recorded as such.
...
docker exec -i "${NODE}" sh -c 'cat > /etc/systemd/system/stargz-snapshotter.service.d/killmode.conf' <<'DROPIN'
[Service]
KillMode=process
DROPIN
docker exec "${NODE}" systemctl daemon-reload

# v6 live fix G2: installing a different binary is NOT enough to switch arms.
# KillMode=process -- our own fix for upstream #2387, and the premise of every
# experiment here -- deliberately keeps the FUSE manager alive across a
# snapshotter restart. The manager is where the filesystem, the accounting index
# and the eviction engine live, so after `systemctl restart` the NEW snapshotter
# is talking to the OLD manager and every metric still comes from the previous
# arm's binary. Measured live: after installing f6547d99 over 9829d7cf and
# restarting, :9110 still served 39 cache series from the 9829d7cf manager.
#
# So a setup that changes the binaries must replace the manager explicitly. This
# is setup, not measurement: E1 tests what a plain restart does, and does its own
# restarts after this point.
if [ -n "$(docker exec "${NODE}" sh -c "pgrep -f '${FM_PAT}'" 2>/dev/null)" ]; then
  log "replacing the running FUSE manager so the new binaries actually take effect"
  docker exec "${NODE}" systemctl stop stargz-snapshotter 2>/dev/null || true
  docker exec "${NODE}" sh -c "pkill -TERM -f '${FM_PAT}' || true"; sleep 4
  docker exec "${NODE}" sh -c "pkill -9 -f '${FM_PAT}' || true"
  docker exec "${NODE}" sh -c 'rm -f /run/containerd-stargz-grpc/fuse-manager.sock'
  sg_umount_stale
...
sleep 6
# Assert the manager actually runs the binary we installed.
want=$(md5sum "${BINSRC}/stargz-fuse-manager" | cut -d" " -f1)
got=$(docker exec "${NODE}" md5sum /usr/local/bin/stargz-fuse-manager 2>/dev/null | cut -d" " -f1)
[ "${want}" = "${got}" ] || die "installed manager md5 ${got} != ${BINSRC} ${want}"
log "manager binary confirmed: ${got} (from ${BINSRC})"
...
log "stargz-snapshotter arm=${ARM_LABEL} up under systemd, fuse_manager=${FUSE_MANAGER}, budget=${CACHE_BUDGET_BYTES} policy=${CACHE_POLICY}"
