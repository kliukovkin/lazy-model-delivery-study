#!/usr/bin/env bash
# spike-v4 rig lifecycle, run from the workstation (NOT on the hosts).
...
#   ./99-aws-lifecycle.sh verify    -> assert no v4 instance is in a billable state
set -euo pipefail
...
up)
  ...
  for NAME in "${TAGS[@]}"; do
    aws ec2 run-instances --image-id "${AMI}" --instance-type "${ITYPE}" \
      ...
      --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=${NAME}},{Key=spike,Value=v4}]" \
      --block-device-mappings '[{"DeviceName":"/dev/sda1","Ebs":{"VolumeSize":30,"VolumeType":"gp3","DeleteOnTermination":true}}]' \
      --query 'Instances[0].InstanceId' --output text
  done
  log "waiting for running state"
  aws ec2 wait instance-running --filters "Name=tag:spike,Values=v4" "Name=instance-state-name,Values=pending,running"
  "$0" ips | tee "${HOSTS_ENV}"
  log "hosts.env written -> ${HOSTS_ENV}"
  ;;
ips)
  aws ec2 describe-instances --filters "Name=tag:spike,Values=v4" "Name=instance-state-name,Values=running" \
    ...
    --output text | while read -r n pub priv id; do
      case "${n}" in
        v4-registry) echo "REG_PUB=${pub}"; echo "REG_PRIV=${priv}"; echo "REG_ID=${id}";;
        v4-node)     echo "NODE_PUB=${pub}"; echo "NODE_PRIV=${priv}"; echo "NODE_ID=${id}";;
      esac
    done
  ;;
down)
  IDS=$(aws ec2 describe-instances --filters "Name=tag:spike,Values=v4" \
...
verify)
  ...
  echo "=== unattached EBS volumes (should be none from this spike) ==="
