#!/usr/bin/env bash
# spike-v4 registry-host: build ONLY what S1-S3 need.
...
set -euo pipefail
REG_PORT=5000
SIZES_GB="${SIZES_GB:-140}"   # 14g dropped: no v4 experiment references it
MODEL_IMG_PREFIX="localhost:${REG_PORT}/model-ballast"
PRED_IMG="localhost:${REG_PORT}/custom-predictor:v4"
...
if ! curl -sf "http://localhost:${REG_PORT}/v2/custom-predictor/manifests/v4" >/dev/null 2>&1; then
  log "building custom-predictor image"
  mkdir -p "${WORK}/predictor"
  cp /home/ubuntu/v4/custom-predictor/predictor.py "${WORK}/predictor/"
  cp /home/ubuntu/v4/custom-predictor/Dockerfile   "${WORK}/predictor/"
