#!/usr/bin/env python3
"""E2 gate and analysis. PRE-REGISTRATION-v8.md s2.3.
...
print("=== GATE ===")
gate_ok = True
# v7 s3.3: an arm that never converges is excluded; more than two exclusions
# means the crossover has nothing left to say.
if len(runs) < 4:
    print("G0 FAIL  only %d valid arm-runs of 4" % len(runs))
    gate_ok = False
else:
    print("G0 PASS  %d of 4 arm-runs valid" % len(runs))
...
if not gate_ok:
    ...
    print("PRE-REGISTRATION-v7.md s3.3 makes this binding: the arms do not share a")
    ...
    with open(os.path.join(E2, "GATE-FAILURE.md"), "w") as f:
        ...
        f.write("No policy comparison was computed. PRE-REGISTRATION-v7.md s3.3 makes\n")
...
print("Hot set is ~70GB against an 80GB budget - 88% of it - which is the regime")
print("v7 s10.1 identified as the only one where the policies can differ.")
print("If this is NOT SEPARABLE, PRE-REGISTRATION-v8 s2.1 makes the negative final.")
print()
...
print()
print("=== EFFECT SIZE (N=2 per policy; PRE-REGISTRATION-v7 s3.4) ===")
...
for q, label in ((50, "p50"), (95, "p95"), (99, "p99")):
    ...
    # With N>2 the within-policy spread is the RANGE, not a single difference.
    within = max(max(lru) - min(lru), max(twq) - min(twq))
    med_l, med_t = sorted(lru)[len(lru) // 2], sorted(twq)[len(twq) // 2]
    ...
    print("%s  lru=%s (mean %.1f)  2q=%s (mean %.1f)  between=%.1f  max_within=%.1f  ->  %s"
          % (label, ["%.1f" % x for x in lru], med_l, ["%.1f" % x for x in twq], med_t,
             between, within, (direction if sep else "NOT SEPARABLE")))

lru_h = by("lru", lambda r: r["hit_pooled"])
twq_h = by("2q",  lambda r: r["hit_pooled"])
if len(lru_h) >= 2 and len(twq_h) >= 2:
    within = max(max(lru_h) - min(lru_h), max(twq_h) - min(twq_h))
    between = abs(sorted(twq_h)[len(twq_h) // 2] - sorted(lru_h)[len(lru_h) // 2])
...
chk("E2.3", all(r["errs"] == 0 for r in runs),
    "resident read errors per run = %s" % [r["errs"] for r in runs])
chk("E2.4", all(r["evictions"] > 0 for r in runs),
    "evictions per run = %s" % [int(r["evictions"]) for r in runs])
chk("E2.x", all(r["skipped"] == 0 for r in runs),
...
lab_ok = all(("lru" in r["labels_end"].split(",")) if r["policy"] == "lru"
             ...
             for r in runs)
chk("E2.5", lab_ok, "end-of-run policy labels = %s" % [(r["idx"], r["policy"], r["labels_end"]) for r in runs])
promoted = []
for r in runs:
    if r["policy"] != "2q":
        continue
...
chk("E2.5b", all(p[1] and p[2] > 0 for p in promoted) if promoted else False,
