#!/usr/bin/env bash
# E2 [P0] -- the decisive RQ1 arm: hot set AT the budget.
#
# v7's E3 found lru and 2q not separable at N=4, and named the reason: a 14GB hot
# set against an 80GB budget is so comfortable that lru retains it anyway, so
# scan resistance has nothing at risk to protect. This puts the hot set at 88% of
# the budget, which is the only regime where the two policies can differ.
#
# Pre-registered: if 2q is not separable here either, the negative is FINAL and
# no further RQ1 measurement will be proposed (PRE-REGISTRATION-v8 s2.1).
#
# Deviation from "sweep 140GB", disclosed in s2.2: the snapshot carries one large
# image, and a 70GB hot set plus a 140GB sweep cannot both come from it AND be
# disjoint. They must be disjoint -- otherwise the sweep's own reads promote the
# hot set's chunks and destroy the distinction being measured. So the resident
# reads files 1-130 (~70GB) and the sweep reads files 131-280 (~80GB), no chunk
# in common, ~150GB working set against an 80GB budget.
#
...
# Both pods use the SAME image and disjoint file ranges; see the header.
RESIDENT_IMG="${MODEL_IMG_PREFIX}:estargz-140g"
SWEEP_IMG="${MODEL_IMG_PREFIX}:estargz-140g"
HOT_FILES="${HOT_FILES:-130}"         # files 1-130, ~70GB, = 88% of the budget
SWEEP_LO="${SWEEP_LO:-130}"           # sweep reads files 131-280, ~80GB
ROUNDS="${ROUNDS:-4}"
# N=4 per policy. v6 ran N=2 and found the within-policy spread (112 ms at p50)
# exceeding the between-policy gap, so N=2 cannot separate these policies
# whatever it measures. Four gives the effect-size rule something to work with
# and leaves a run spare if one arm is invalidated.
ARM_SEQUENCE="${ARM_SEQUENCE:-lru 2q 2q lru}"   # N=2 per policy, ABBA
E2="${RESULTS_DIR}/e2"
NODE="$(NODE_CTR)"
...
armrun() { # armrun <seq-index> <policy>
  ...
  # Disjoint range: files 131-280. full_read would start at 0 and overlap the
  # resident's hot set, promoting its chunks and erasing what 2q is being asked
  # to demonstrate.
  ( mount_probe e2-sweep "${SWEEP_LO}" 280 > "${out}/sweep-read.txt" 2>&1 ) </dev/null >/dev/null 2>&1 &
  local SWEEPPID=$!
  ...
  # v7 rule 1: an experiment that has not been collected is not finished. v6
  # lost every E3 and E4 bundle to a watchdog that fired before its single
  # end-of-session collection step ran.
  bash "$(dirname "$0")/91-collect-one.sh" "e2/run${idx}-${pol}" || log "  WARN: collection failed for run ${idx}"
}
