#!/usr/bin/env bash
# spike-v8 rig lifecycle, run from the workstation (NOT on the hosts).
...
up)
  ...
  for NAME in "${TAGS[@]}"; do
    ...
    if [ "${NAME}" = "v8-registry" ]; then
      # Restored from the v7 artifact snapshot, not blank: the 150GB of images
      # is already in it, so v8 skips the ~1h40m build that v5, v6 and v7 each
      # paid. SnapshotId on a block-device-mapping is the cheapest possible
      # restore - no copy, no separate create-volume/attach dance.
      BDM='[{"DeviceName":"/dev/sda1","Ebs":{"VolumeSize":30,"VolumeType":"gp3","DeleteOnTermination":true}},
            {"DeviceName":"/dev/sdf","Ebs":{"SnapshotId":"'"${ARTIFACT_SNAPSHOT:-snap-0118cc5716e9e8a54}"'","VolumeSize":1200,"VolumeType":"gp3","Throughput":750,"Iops":8000,"DeleteOnTermination":true}}]'
    fi
    aws ec2 run-instances --image-id "${AMI}" --instance-type "${ITYPE}" \
      --key-name "${KEY_NAME}" --security-group-ids "${SG_ID}" \
      --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=${NAME}},{Key=spike,Value=v8}]" \
  ...
  aws ec2 wait instance-running --filters "Name=tag:spike,Values=v8" "Name=instance-state-name,Values=pending,running"
  "$0" ips | tee "${HOSTS_ENV}"
  log "hosts.env written -> ${HOSTS_ENV}"
  ;;
ips)
  aws ec2 describe-instances --filters "Name=tag:spike,Values=v8" "Name=instance-state-name,Values=running" \
    ...
    --output text | while read -r n pub priv id; do
      case "${n}" in
        v8-registry) echo "REG_PUB=${pub}"; echo "REG_PRIV=${priv}"; echo "REG_ID=${id}";;
        v8-node)     echo "NODE_PUB=${pub}"; echo "NODE_PRIV=${priv}"; echo "NODE_ID=${id}";;
      esac
    done
  ;;
down)
  IDS=$(aws ec2 describe-instances --filters "Name=tag:spike,Values=v8" \
...
verify)
  ...
  echo "=== ALL EBS volumes, any state (v8 requires zero) ==="
  ...
  echo "=== snapshots owned by this account (v8 keeps EXACTLY ONE: the v7 artifact snapshot) ==="
  ...
  echo "=== AMIs owned by this account (v8 requires zero) ==="
