...
ACCOUNTING_STANZA=""
if [ "${ARM_LABEL}" = "ours" ] && [ "${CACHE_ACCOUNTING}" = "true" ]; then
...
log "writing /etc/containerd-stargz-grpc/config.toml (arm=${ARM_LABEL} fuse_manager=${FUSE_MANAGER})"
...
[fuse_manager]
  enable = ${FUSE_MANAGER}
  # v5 finding 1: without this the accounting/eviction metrics are unreachable,
  # because they are registered in the manager process, not in this one.
  metrics_address = "${STARGZ_FM_METRICS_ADDRESS}"
