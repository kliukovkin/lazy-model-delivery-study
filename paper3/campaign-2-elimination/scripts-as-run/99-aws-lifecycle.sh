#!/usr/bin/env bash
# spike-v5 rig lifecycle, run from the workstation (NOT on the hosts).
...
up)
  ...
  for NAME in "${TAGS[@]}"; do
    aws ec2 run-instances --image-id "${AMI}" --instance-type "${ITYPE}" \
      ...
      --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=${NAME}},{Key=spike,Value=v5}]" \
  ...
  aws ec2 wait instance-running --filters "Name=tag:spike,Values=v5" "Name=instance-state-name,Values=pending,running"
  "$0" ips | tee "${HOSTS_ENV}"
  log "hosts.env written -> ${HOSTS_ENV}"
  ;;
ips)
  aws ec2 describe-instances --filters "Name=tag:spike,Values=v5" "Name=instance-state-name,Values=running" \
    ...
    --output text | while read -r n pub priv id; do
      case "${n}" in
        v5-registry) echo "REG_PUB=${pub}"; echo "REG_PRIV=${priv}"; echo "REG_ID=${id}";;
        v5-node)     echo "NODE_PUB=${pub}"; echo "NODE_PRIV=${priv}"; echo "NODE_ID=${id}";;
      esac
    done
  ;;
down)
  IDS=$(aws ec2 describe-instances --filters "Name=tag:spike,Values=v5" \
