...
# ---------------------------------------------------------------- stargz chunk-cache: REAL partition (v3.1)
# v3 used a loopback file here; red-team pushback: "you hid the fs from
# kubelet by hand." v3.1 uses a real block device (/dev/nvme1n1p2, partitioned
# in 00-node-host-setup.sh) mounted at STARGZ_CACHE_MOUNT (/cache-part) --
# a real, independently-visible filesystem, bind-mounted into the kind node
# via extraMounts the same way the v3 loopback was. Must exist BEFORE
# `kind create cluster` (extraMounts is fixed at cluster-creation time).
...
# NOTE (live pitfall, v3.1): originally also tried a SECOND cache root on big
# /data (P0.6 "sufficient cache" full-read wanted headroom beyond the 100GB
# partition) via a second stargz-grpc instance / extraMount / bind-mount
# swap. After several hours of genuinely bizarre, hard-to-pin-down breakage
# (containerd proxy_plugins registration silently reverting, snapshot-dir
# ENOENT after manual mount surgery, "AlreadyExists" snapshot leases,
# repeated 900s Ready timeouts) across FOUR clean-looking fix attempts, gave
# up on the dual-cache-root approach entirely -- too fragile to trust for
# real measurements. P0.6's 140GB "sufficient cache" full-read sub-goal is
# NOT achieved in this run; documented as dropped-for-time in the report.
# Single cache mount only, matching what the rest of the P0 experiments need.
